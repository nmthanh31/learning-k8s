...
* Chịu tải lớn, thích hợp cluster production.
* Health check tăng độ tin cậy.
* Dễ mở rộng node control plane mà không gián đoạn kết nối.

**Nhược điểm:**

* Thêm phần mềm/hardware cần quản lý.
* Nếu LB không HA, sẽ là nút thắt toàn cluster.

---

### 5.2.4. Khi nào nên dùng

* Cluster production nhiều node control plane hoặc worker.
* Triển khai trên cloud hoặc on-prem có LB HA.
* Yêu cầu ổn định, cân bằng tải thực sự.
...
* Triển khai nhanh, ít phụ thuộc thiết bị ngoài.
* Tiết kiệm tài nguyên, phù hợp cluster nhỏ hoặc lab.

**Nhược điểm:**

* Chỉ 1 node Active xử lý traffic → không cân bằng tải thực sự.
* Failover mất 1–3 giây, có thể gián đoạn request đang xử lý.
* Hạn chế mở rộng khi traffic lớn.

---

### 5.3.5. Khi nào nên dùng

* Lab, test, cluster nhỏ (≤3 master nodes).
* On-premise hoặc Edge, hạ tầng hạn chế.
* Muốn triển khai nhanh, đơn giản, ít phụ thuộc quản trị mạng.
